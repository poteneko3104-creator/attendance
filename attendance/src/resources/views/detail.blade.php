...
<div class="attendance-container">
    ...
    <form action="/detail" method="POST" class="detail-card">
        ...
        @foreach($date->attendance()->where('status',$status)->where('category', '出勤')->get() as $key => $attendance)
        <div class="form-group">           
            ...
            <div class="form-value time-range">
                ...
                <input type="text" name="new_attendances[{{$key}}][start_time]" class="input-time text-center" value="{{ $attendance->start_time->format('H:i') }}">
                <span class="tilde">〜</span>
                <input type="text" name="new_attendances[{{$key}}][end_time]" class="input-time text-center" value="{{ $attendance->end_time?->format('H:i') }}">
            </div>
        ...
        @endforeach
        @php
            $attendanceCount = $date->attendance->where('status',$status)->where('category','出勤')->count();
        @endphp
        
        @foreach($date->attendance()->where('status', $status)->where('category', '休憩')->get() as $attendance)
        @php
        ...
        <div class="form-group">           
            ...
            <div class="form-value time-range">
                ...
                <input type="text" name="new_attendances[{{$currentIndex}}][start_time]" class="input-time text-center" value="{{ $attendance->start_time->format('H:i') }}">
                <span class="tilde">〜</span>
                <input type="text" name="new_attendances[{{$currentIndex}}][end_time]" class="input-time text-center" value="{{ $attendance->end_time?->format('H:i') }}">
            </div>
        ...
        @php 
             $totalCount = $attendanceCount + $date->attendance->where('status',$status)->where('category', '休憩')->count(); 
        @endphp
        ...
        <div class="form-group">
            ...
            <div class="form-value time-range">
                ...
                <input type="text" class="input-time text-center" name="new_attendances[{{$totalCount}}][start_time]" value="">
                <span class="tilde">〜</span>
                <input type="text" class="input-time text-center" name="new_attendances[{{$totalCount}}][end_time]" value="">
            </div>
        ...
        <div class="form-group">
            ...
            <div class="form-value">
                <input type="text" class="input-remarks" name="remarks" value="{{$date->remarks}}">
            </div>
            @error('remarks')
            <div class="aleart-message" >
                {{ $message }}
